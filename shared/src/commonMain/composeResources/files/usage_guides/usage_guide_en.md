...
Below the action row is a card with the scramble and a **`New`** button for generating a new scramble. Below that, in the lower part of the screen, is the timer and finally a quick overview of statistics: fluency, Ao5 and Ao12 for
...
- **pencil icon** (next to the cube's name) - renames the cube. Leave the field blank and save to go back to the name the cube reports about itself. The name you choose belongs to the profile you're using - if you share the phone, other profiles keep their own name for the same cube.
